...
void setup() 
{
  ...
  Serial.print("\n");
  Serial.print("Welcome to ECE516 Lab 1.\n");
...
void loop() 
{
  ...
  for (int iy = 0; iy < 120; iy++)
  {
    ...
    {
      mtx[ix][iy] =   fb->buf[((ix*2)+1)+(iy*320) ];      //make simplified-access copy of the framebuffer in LOCAL ram
      
      mtx[ix][iy] |=  (fb->buf[((ix*2)+0)+(iy*320) ])<<8; //make simplified-access copy of the framebuffer in LOCAL ram
    }
  }
  ...
  for (int iy=0; iy<(120); iy=iy+1)    //process and write to outbound buffer
  {
    for (int ix=0; ix<(160); ix=ix+1) 
    {
      ...
      if ((framenumber<61)&&(framenumber>59))
      {
        Serial.print(mred);
        Serial.print(" ");
        Serial.print(mgreen);
    ...
    if ((framenumber<61)&&(framenumber>59)) Serial.print("\n");
  }
  if ((framenumber<61)&&(framenumber>59)) Serial.print("\n\n");
  framenumber++; 
}
